...
public class ConjurAPI {
	...
	public static String getAuthorizationToken(OkHttpClient client, ConjurConfiguration configuration,
		...
		LOGGER.log(Level.FINE, "inside GetAuthorization Token");
		...
		if (context != null) {
			LOGGER.log(Level.FINE, "Context not null inside getAuthorization token" + availableCredentials.toString());

			LOGGER.log(Level.FINE, "Context not null inside getAuthorization token" + context.getClass());

			if (context instanceof Run) {
				LOGGER.log(Level.FINE,
						"Context instance of Run getAuthorization token" + availableCredentials.toString());
				...
				LOGGER.log(Level.FINE,
						"Context not instance of Run getAuthorization token" + ((AbstractItem) context).getParent());

				// availableCredentials.addAll(CredentialsProvider.lookupCredentials(UsernamePasswordCredentials.class,
				// (AbstractItem) context, ACL.SYSTEM,
				// Collections.<DomainRequirement>emptyList()));
				availableCredentials.addAll(CredentialsProvider.lookupCredentials(UsernamePasswordCredentials.class,
						((AbstractItem) context).getParent(), ACL.SYSTEM, Collections.<DomainRequirement>emptyList()));
		...
		Request request = buildRequest(conjurAuthn);

		/*
		 * Request request = null; if (conjurAuthn.login != null && conjurAuthn.apiKey
		 * != null) { LOGGER.log(Level.FINE, "Authenticating with Conjur (authn)");
		 * request = new Request.Builder()
		 * .url(String.format("%s/%s/%s/%s/authenticate", conjurAuthn.applianceUrl,
		 * conjurAuthn.authnPath, conjurAuthn.account,
		 * URLEncoder.encode(conjurAuthn.login, "utf-8")))
		 * .post(RequestBody.create(MediaType.parse("text/plain"),
		 * conjurAuthn.apiKey)).build(); } else if (conjurAuthn.authnPath != null &
		 * conjurAuthn.apiKey != null) { String authnPath =
		 * conjurAuthn.authnPath.indexOf("/") == -1 ? "authn-jwt/" +
		 * conjurAuthn.authnPath : conjurAuthn.authnPath; LOGGER.log(Level.FINE,
		 * "Authenticating with Conjur (JWT) authnPath={0}", authnPath); request = new
		 * Request.Builder() .url(String.format("%s/%s/%s/authenticate",
		 * conjurAuthn.applianceUrl, authnPath, conjurAuthn.account))
		 * .post(RequestBody.create(MediaType.parse("text/plain"),
		 * conjurAuthn.apiKey)).build();
		 * 
		 * }
		 */
		...
		if (request != null) {
			...
			if (response.code() != 200) {
				LOGGER.log(Level.FINEST, "Inside faile authentication");
				// + ((AbstractItem) ((AbstractItem) context).getParent()).getParent());
				/*List<Item> itemList = Jenkins.get().getAllItems();
				LOGGER.log(Level.FINE, "build details" + context+"Context Class"+context.getClass());
				Item toCheck = null;
				for (Item item : itemList) {
					LOGGER.log(Level.FINE,
							"All items in the list" + item.getDisplayName()+ "Context" + context.toString());
				...
				if (context != null) {
					LOGGER.log(Level.FINE, "Get jobs inside Conjur APIs" + context.getDisplayName());
					...
					LOGGER.log(Level.FINE, "Child Folder in Conjur API" + childFolder + ">>>>>>" + spiltJob.length);

					for (int i = 0; i < spiltJob.length; i++) {

						LOGGER.log(Level.FINE, "Conjur API new logic", parentFolder.getDisplayName() + "Level" + i);

						ConjurAuthnInfo conjurAuthn1 = getConjurAuthnInfo(configuration, availableCredentials,
								parentFolder);
						Request req = buildRequest(conjurAuthn1);
						Response res = null;
						if (req != null) {

							 res = client.newCall(req).execute();
							resultingToken = Base64.getEncoder().withoutPadding()
									.encodeToString(res.body().string().getBytes("UTF-8"));
							if (res.code() != 200) {
						...
						if (res.code() == 200) {
							LOGGER.log(Level.FINEST,
									() -> "Conjur Authenticate response 200 OK ");
				...
				// throw new IOException("Error authenticating to Conjur [" + response.code() +
				// " - " + response.message()
				// + "\n" + resultingToken);
	...
	private static Request buildRequest(ConjurAuthnInfo conjurAuthn) {
	...
	private static void setConjurAuthnForJITCredentialAccess(ModelObject context, ConjurAuthnInfo conjurAuthn) {
		...
		LOGGER.log(Level.FINE, "setConjurAuthnForJITCredentialAccess >>>"+context);
