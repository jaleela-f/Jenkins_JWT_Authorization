...
public interface ConjurSecretCredentials extends StandardCredentials {
	...
	static ConjurSecretCredentials credentialFromContextIfNeeded(ConjurSecretCredentials credential, String credentialID, ModelObject context) {
		if (credential == null && context != null) {
			...
			if (context instanceof Run) {
				...
				LOGGER.log(Level.FINE,"Inside not conjur credentials"+((AbstractItem)((AbstractItem)context).getParent()).getFullName());
				
				folder = Jenkins.get().getItemByFullName(((AbstractItem)((AbstractItem)context).getParent()).getFullName());
			...
			credential = CredentialsMatchers
					.firstOrNull(
							...
							CredentialsMatchers.withId(credentialID.trim()));
	...
	static ConjurSecretCredentials credentialWithID(String credentialID, ModelObject context) {
		...
		//LOGGER.log(Level.FINE,"* Context Id:"+ context.getDisplayName());
		...
		if (credential == null) {
			...
			String contextLevel = String.format("Unable to find credential at %s", context.getDisplayName());
